...
void CmdLine::init (){
    ...
    for(size_t iarg = 1; iarg < __arguments.size(); iarg++){
      ...
      std::string arg = __arguments[iarg];
  ...
  bool CmdLine::present(const std::string & opt) const {
  ...
  std::string CmdLine::string_val(const std::string & opt, const std::string & defval) const {
  ...
  // have string value...
  int CmdLine::int_val(const std::string & opt) const {
    int result;
    std::string optstring = string_val(opt);
    std::istringstream optstream(optstring);
    optstream >> result;
    if (optstream.fail()) {
      std::cerr << "Error: could not convert option ("<<opt<<") value ("
		<<optstring<<") to int"<<std::endl;
      exit(-1);}
    return result;
  }

  // as above, but if opt is not present_and_set, return default
  int CmdLine::int_val(const std::string & opt, const int & defval) const {
  ...
  // Not too sure what happens if option is present_and_set but does not
  // have string value...
  double CmdLine::double_val(const std::string & opt) const {
    double result;
    std::string optstring = string_val(opt);
    std::istringstream optstream(optstring);
    optstream >> result;
    if (optstream.fail()) {
      std::cerr << "Error: could not convert option ("<<opt<<") value ("
		<<optstring<<") to double"<<std::endl;
      exit(-1);}
    return result;
  }

  // as above, but if opt is not present_and_set, return default
  double CmdLine::double_val(const std::string & opt, const double & defval) const {
  ...
  std::string CmdLine::command_line() const {
  ...
  bool CmdLine::all_options_used() const {
    bool result = true;
    for(std::map<std::string,bool>::const_iterator opt = __options_used.begin();
	opt != __options_used.end(); opt++) {
      bool this_one = opt->second;
      if (! this_one) {std::cerr << "Option "<<opt->first<<" unused/unrecognized"<<std::endl;}
  ...
  void CmdLine::_report_conversion_failure(const std::string & opt,
					   const std::string & optstring) const {
    std::cerr << "Error: could not convert option ("<<opt<<") value ("
	      <<optstring<<") to requested type"<<std::endl;
    exit(-1);
  }
